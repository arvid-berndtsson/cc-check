...
fn install_hook(no_build: bool) -> Result<()> {
    ...
    // Git hooks should always be named "commit-msg" (no extension) on all platforms
    let hook_filename = "commit-msg";
    ...
    // Create hook script (shell script works on all platforms via Git Bash on Windows)
    let hook_content = create_unix_hook(&binary_path)?;

 ...
 fn main() -> Result<()> {
     ...
     let message = if let Some(path) = cli.commit_msg_file {
         ...
         if let Ok(contents) = fs::read_to_string(".git/COMMIT_EDITMSG") {
             contents
         } else {
     ...
     let res = validate_header(&header, &allowed_types, cli.max_subject, cli.no_trailing_period);
 ...
 fn exit_with(format: OutputFormat, res: std::result::Result<(), ValidationError>) -> Result<()> {
     match (format, res) {
         ...
         (OutputFormat::Json, Ok(())) => {
             println!("{}", serde_json::to_string(&JsonResult { ok: true, error: None })?);
             Ok(())
         }
         (OutputFormat::Json, Err(err)) => {
             println!(
                 "{}",
                 serde_json::to_string(&JsonResult { ok: false, error: Some(&err.to_string()) })?
             );
             std::process::exit(1);
         }
     }
